...
function DeleteCargo({ cargo, onDeleteCargo }) {
  const [isDeleteCargoModalOpen, setIsDeleteCargoModalOpen] = useState(false);
  const [deleteCargo, setDeleteCargo] = useState({
    name: "",
    code: "",
  });
  ...
  }, [isDeleteCargoModalOpen]);

  useEffect(() => {
    setDeleteCargo(cargo);
  }, [cargo]);
  ...
  const handleSubmit = () => {
    onDeleteCargo({ ...deleteCargo, id: cargo.id });
    closeModal();
